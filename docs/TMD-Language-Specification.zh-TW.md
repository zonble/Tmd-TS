...
- **首調唱名基準（`?=`）**：設定簡譜音級 `1` 對應的音高。`?= C` 代表 `1=C`，
  `?= D` 代表 `1=D`；這是播放／音高解析脈絡，不代表樂曲是大調或小調。
- **明確調性宣告（`key=` 或 `Key=`）**：正式宣告樂曲的調與模式，例如 `key= Bm`、
  `key= C`、`key= F#m`。它和 `?=` 分開保存，檔頭值保存於 `Sheet.declaredKey`。
- **播放轉換**：`{?+5}`、`{?-2}`、`{?=D}` 改變首調唱名的播放脈絡；不能只因位移改變
  就當成偵測到音樂上的轉調。
...
- 絕對速度（`{!=140}`）與相對速度（`{!+10}`）
- 首調唱名基準（`{?=D}`）與相對移調（`{?+2}`、`{?-2}`）
- 明確調性切換（`{key= Bm}` 或 `{Key= F#m}`），獨立於首調唱名基準保存
- 強弱記號（`{ppp}`、`{pp}`、`{p}`、`{mp}`、`{mf}`、`{f}`、`{ff}`、`{fff}`），影響後續播放與樂譜輸出
...
內容會保存為 `Order.relative` 的字串，例如 `-3` 或 `+3`。絕對首調唱名基準變更使用：
...
這些 `?` 指令只改變播放脈絡，不是實際調性的宣告，也不能單獨當成分析出的轉調。若要宣告樂曲調性，請在檔頭使用 `key=`，或在段落內使用 `{key=...}`。
